...
# Step 1) 
# Two intersecting spheres in 3D with a POV along z axis. 
...
plt.tight_layout()
plt.savefig("./out/part1.png", dpi=dpi, transparent=True)
plt.clf()

# Step 2) 
# Display points calculated on sphere surface, only on face visible to viewer. Color points that are inside other sphere different color.
...
x = sphere1_x_center + radius * np.outer(np.cos(u), np.sin(v))
y = sphere1_y_center + radius * np.outer(np.sin(u), np.sin(v))
z = sphere1_z_center + radius * np.outer(np.ones(np.size(u)), np.cos(v))
ax.plot_surface(x, y, z, color='blue', alpha=0.25)

# Sphere 2
sphere2_radius = 0.75
sphere2_x_center, sphere2_y_center, sphere2_z_center = 0.25, 0, 0.5
x = sphere2_x_center + radius * np.outer(np.cos(u), np.sin(v))
y = sphere2_y_center + radius * np.outer(np.sin(u), np.sin(v))
z = sphere2_z_center + radius * np.outer(np.ones(np.size(u)), np.cos(v))
ax.plot_surface(x, y, z, color='red', alpha=0.25)
...
ax.set_box_aspect([1, 1, 1])
plt.tight_layout()
plt.savefig("./out/part2.png", dpi=dpi, transparent=True)
plt.clf()

# Step 3) 
# Display points from pov and visualize the quickhul algorithm and that an svg is made from it.
...
plt.tight_layout()
plt.savefig("./out/part3.png", dpi=dpi, transparent=True)
plt.clf()

# Step 4) Quickhull on points to generate convex hull.
# Plot visible points on 2D.
fig = plt.figure()
ax = fig.add_subplot(111)
ax.axis('off')
...
plt.gca().set_aspect('equal')
plt.tight_layout()
plt.savefig("./out/part4.png", dpi=dpi, transparent=True)
plt.clf()

# Step 5) Plot sphere 1 and the convex hull on top of it.
fig = plt.figure()
ax = fig.add_subplot(111)
ax.axis('off')
ax.set_xticks([])
...
plt.gca().set_aspect('equal')
plt.tight_layout()
plt.savefig("./out/part5.png", dpi=dpi, transparent=True)
plt.clf()