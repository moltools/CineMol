...
module Drawing =
    ...
    let draw
        ...
        let atoms = atoms |> Array.sortBy (fun atom -> atom.Center.Distance pov |> (*) -1.0)

        // Recalculate radius points based on distance point to pov
        let atoms = atoms |> Array.map (fun atom ->
            let newRadius = (distPovOrigin / (pov.Distance atom.Center)) * atom.Radius
            { atom with Radius = newRadius })
        ...
        let setPerspective (atom: AtomInfo) : AtomInfo =
            let newCenter = project cameraPerpendicular cameraHorizon cameraForward (pov: Point) focalLength atom.Center
            { atom with Center = newCenter }
