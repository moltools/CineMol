...
def permute(f: Callable[[int], int], array: Array[_T]) -> Array[_T]:
    ...
    def _arrow47(i: int, x: _T, f: Any = f, array: Any = array) -> None:
        j: int = f(i) or 0
    ...
    iterate_indexed(_arrow47, array)
...
def sort_in_place_by(projection: Callable[[_A], _B], xs: Array[_A], comparer: IComparer_1[_B]) -> None:
    def _arrow48(x: _A, y: _A, projection: Any = projection, xs: Any = xs, comparer: Any = comparer) -> int:
        return comparer.Compare(projection(x), projection(y))

    xs.sort(key=functools.cmp_to_key(_arrow48))


def sort_in_place(xs: Array[_T], comparer: IComparer_1[_T]) -> None:
    def _arrow49(x: _T, y: _T, xs: Any = xs, comparer: Any = comparer) -> int:
        return comparer.Compare(x, y)

    xs.sort(key=functools.cmp_to_key(_arrow49))


def sort_in_place_with(comparer: Callable[[_T, _T], int], xs: Array[_T]) -> Array[_T]:
...
def sort_descending(xs: Array[_T], comparer: IComparer_1[_T]) -> Array[_T]:
    def _arrow52(x: _T, y: _T, xs: Any = xs, comparer: Any = comparer) -> int:
        return comparer.Compare(x, y) * -1

    return sorted(xs, key=functools.cmp_to_key(_arrow52))


def sort_by_descending(projection: Callable[[_A], _B], xs: Array[_A], comparer: IComparer_1[_B]) -> Array[_A]:
    def _arrow53(x: _A, y: _A, projection: Any = projection, xs: Any = xs, comparer: Any = comparer) -> int:
        return comparer.Compare(projection(x), projection(y)) * -1

    return sorted(xs, key=functools.cmp_to_key(_arrow53))
...
def unzip(array: Array[tuple[__A, __B]]) -> tuple[Array[__A], Array[__B]]:
    ...
    def _arrow54(i: int, tupled_arg: tuple[__A, __B], array: Any = array) -> None:
        res1[i] = tupled_arg[0]
        res2[i] = tupled_arg[1]

    iterate_indexed(_arrow54, array)
    return (res1, res2)
...
def unzip3(array: Array[tuple[__A, __B, __C]]) -> tuple[Array[__A], Array[__B], Array[__C]]:
    ...
    def _arrow55(i: int, tupled_arg: tuple[__A, __B, __C], array: Any = array) -> None:
        res1[i] = tupled_arg[0]
        res2[i] = tupled_arg[1]
        res3[i] = tupled_arg[2]
...
def fold_back_indexed(folder: Callable[[int, _T, _STATE], _STATE], array: Array[_T], state: _STATE) -> _STATE:
    ...
    return array.reduceRight((_arrow56, state))


def fold_back(folder: Callable[[_T, _STATE], _STATE], array: Array[_T], state: _STATE) -> _STATE:
    def _arrow57(acc: _STATE, x: _T, folder: Any = folder, array: Any = array, state: Any = state) -> _STATE:
        return folder(x, acc)

    return functools.reduce(_arrow57, array[::-1], state)
...
def fold2(
    ...
    return fold_indexed2(_arrow58, state, array1, array2)
...
def fold_back2(
    ...
    return fold_back_indexed2(_arrow59, array1, array2, state)
...
def for_all2(predicate: Callable[[__A, __B], bool], array1: Array[__A], array2: Array[__B]) -> bool:
    ...
    return fold2(_arrow60, True, array1, array2)
...
def min_by(projection: Callable[[_A], _B], xs: Array[_A], comparer: IComparer_1[_B]) -> _A:
    def _arrow63(x: _A, y: _A, projection: Any = projection, xs: Any = xs, comparer: Any = comparer) -> _A:
...
def min(xs: Array[_A], comparer: IComparer_1[_A]) -> _A:
    def _arrow64(x: _A, y: _A, xs: Any = xs, comparer: Any = comparer) -> _A:
...
def windowed(window_size: int, source: Array[_T]) -> Array[Array[_T]]:
    ...
    def _arrow65(x: int, y: int, window_size: Any = window_size, source: Any = source) -> int:
        return compare_primitives(x, y)

    len_1: int = max_1(_arrow65, 0, (len(source) - window_size) + 1) or 0
    res = [None] * len_1
...
def split_into(chunks: int, array: Array[_T]) -> Array[Array[_T]]:
    ...
    if len(array) == 0:
        ...
        def _arrow66(x: int, y: int, chunks: Any = chunks, array: Any = array) -> int:
            return compare_primitives(x, y)

        chunks_1: int = min_1(_arrow66, chunks, len(array)) or 0
        ...
        for i in range(0, (chunks_1 - 1) + 1, 1):
            ...
            def _arrow67(x_1: int, y_1: int, chunks: Any = chunks, array: Any = array) -> int:
                return compare_primitives(x_1, y_1)

            start: int = ((i * min_chunk_size) + min_1(_arrow67, chunks_with_extra_item, i)) or 0
            slice: Array[_T] = array[start : start + chunk_size]
            ignore(result.append(slice))
        return result
