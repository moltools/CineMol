...
let writeBondsStyle (atoms: AtomInfo[]) (bonds: BondInfo[]) : string =
    bonds
    |> Array.map (fun bond -> $"\n.bond-{bond.Index}{{fill:url(#radial-gradient-{atoms.Length + bond.Index});}}")
    |> String.concat ""

let writeBondsDefs (atoms: AtomInfo[]) (bonds: BondInfo[]) : string =
    ...
    |> Array.map (fun bond ->
        ...
        | Some s, Some e ->
            ...
            let centroid = sProj.Centroid eProj
            let length = sProj.Distance eProj
            $"\n<radialGradient\
            \n\tid=\"radial-gradient-{atoms.Length + bond.Index}\"\
            \n\tcx=\"{floatToStr centroid.X}\"\
            \n\tcy=\"{floatToStr centroid.Y}\"\
            \n\tfx=\"{floatToStr centroid.X}\"\
            \n\tfy=\"{floatToStr centroid.Y}\"\
            \n\tr=\"{floatToStr (length + 0.4)}\"\
            \n\tgradientTransform=\"matrix(1, 0, 0, 1, 0, 0)\"\
            \n\tgradientUnits=\"userSpaceOnUse\"\
            \n>\
            \n<stop offset=\"{floatToStr (1.0 - atomColorGradient.[0])}\" \
...
let writeBallAndStick (atoms: AtomInfo[]) (bonds: BondInfo[]) : string =
    let atoms = Array.toList atoms
    ...
    let drawBond (bond: BondInfo) (s: AtomInfo) (e: AtomInfo) : string =
        let width = 0.5
        let sProj = { X = s.Center.X; Y = s.Center.Y }
        let eProj = { X = e.Center.X; Y = e.Center.Y }
        ...
        // \n\t\tA {floatToStr width} {floatToStr (width / 5.0)} 0 0 0 {floatToStr sBot.X} {floatToStr sBot.Y}\
        // \n\t\tL {floatToStr eBot.X} {floatToStr eBot.Y}\
        // \n\t\tA {floatToStr width} {floatToStr (width / 5.0)} 0 0 0 {floatToStr eTop.X} {floatToStr eTop.Y}\
        ...
        \n\td= \
        \n\t\t\"M {floatToStr sTop.X} {floatToStr sTop.Y}\
        \n\t\tL {floatToStr sBot.X} {floatToStr sBot.Y}\
        \n\t\tL {floatToStr eBot.X} {floatToStr eBot.Y}\
        \n\t\tL {floatToStr eTop.X} {floatToStr eTop.Y}\
        \n\t\tL {floatToStr sTop.X} {floatToStr sTop.Y}\"\
        />\n"

    let rec findAtom (l: AtomInfo list) atomIdx : AtomInfo option =
        match l with
        | [] -> None
